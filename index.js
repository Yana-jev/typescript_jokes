...
const output = document.getElementById('joke-container');
const nextJoke = document.getElementById('getJoke');
const blobMain = document.querySelector('.blob-main');
const api = 'https://icanhazdadjoke.com/';
const api2 = 'https://api.chucknorris.io/jokes/random';
const divertidoBtn = document.getElementById("button_good");
const neutralBtn = document.getElementById("button_normal");
const tristeBtn = document.getElementById("button_bad");
let now = new Date();
let score = 0;
let reportAcudits = { joke: "", score: 0, date: now.toISOString() };
let reportJokes = [];
function getJoke() {
...
function getChuckNorrisJoke() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield fetch(api2);
            if (!response.ok) {
...
function meteoCurrent() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield fetch("https://api.openweathermap.org/data/2.5/weather?lat=41.3888&lon=2.159&appid=6ac23fcea84514231e680bd47fa91be3&units=metric");
            if (response.ok) {
                const data = yield response.json();
                console.log(data);
